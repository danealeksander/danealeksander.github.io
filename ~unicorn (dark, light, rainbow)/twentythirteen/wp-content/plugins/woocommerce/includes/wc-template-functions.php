...
function wc_products_rss_feed() {
	...
	if ( is_post_type_archive( 'product' ) || is_singular( 'product' ) ) {
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_attr__( 'New works:', 'woocommerce' ) . '" href="' . esc_url( $feed ) . '" />';
		...
		if ( $term ) {
			...
			echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf( esc_attr__( 'New works added to %s', 'woocommerce' ), $term->name ) . '" href="' . esc_url( $feed ) . '" />';
		}
		...
		if ( $term ) {
			...
			echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf( esc_attr__( 'New works tagged %s', 'woocommerce' ), urlencode( $term->name ) ) . '" href="' . esc_url( $feed ) . '" />';
		}
	}
}
...
if ( ! function_exists( 'woocommerce_taxonomy_archive_description' ) ) {
	...
	function woocommerce_taxonomy_archive_description() {
		if ( is_product_taxonomy() && 0 === absint( get_query_var( 'paged' ) ) ) {
			...
			if ( $description ) {
				echo '<h5 class="term-description">' . $description . '</h5>';
			}
		}
	}
}
...
if ( ! function_exists( 'woocommerce_breadcrumb' ) ) {
	...
	function woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			'delimiter'   => '',
			'wrap_before' => '<nav><ul class="single">',
			'wrap_after'  => '</ul></nav>',
			'before'      => '',
			'after'       => '</a></li>',
