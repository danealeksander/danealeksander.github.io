...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message   = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( $return_to ), esc_html__( 'Back to Shop', 'woocommerce' ), esc_html( $added_text ) );
	} else {
...
function wc_cart_totals_coupon_label( $coupon, $echo = true ) {
	...
	$label = apply_filters( 'woocommerce_cart_totals_coupon_label', sprintf( esc_html__( 'Discount: %s', 'woocommerce' ), $coupon->get_code() ), $coupon );
...
function wc_cart_totals_coupon_html( $coupon ) {
	...
	$coupon_html          = $discount_amount_html . ' [<a href="' . esc_url( add_query_arg( 'remove_coupon', urlencode( $coupon->get_code() ), defined( 'WOOCOMMERCE_CHECKOUT' ) ? wc_get_checkout_url() : wc_get_cart_url() ) ) . '" class="woocommerce-remove-coupon" data-coupon="' . esc_attr( $coupon->get_code() ) . '">' . __( 'Undo', 'woocommerce' ) . '</a>]';
...
function wc_cart_totals_order_total_html() {
	$value = '<font class="underscore">' . WC()->cart->get_total() . '</font>';
