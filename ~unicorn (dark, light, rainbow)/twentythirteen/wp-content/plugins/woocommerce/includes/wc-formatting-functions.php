...
function wc_price( $price, $args = array() ) {
	...
	$formatted_price = ( $negative ? '-' : '' ) . sprintf( $price_format, '<font class="woo-currency">' . get_woocommerce_currency_symbol( $currency ) . '</font>', $price );
...
function wc_format_stock_for_display( $product ) {
	...
	if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
		$display .= ' ' . __( '(Available on backorder.)', 'woocommerce' );
	}
