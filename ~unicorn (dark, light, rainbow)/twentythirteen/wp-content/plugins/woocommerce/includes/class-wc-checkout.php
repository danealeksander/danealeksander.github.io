...
class WC_Checkout {
	...
	public function __clone() {
		wc_doing_it_wrong( __FUNCTION__, __( 'Take a wrong turn?', 'woocommerce' ), '2.1' );
	}
		...
		if ( is_null( $this->fields ) ) {
			$this->fields = array(
				...
				'order'    => array(
					'order_comments' => array(
						...
						'placeholder' => esc_attr__( 'Notes about your order, for example special notice for delivery.', 'woocommerce' ),
					),
				),
			);
			if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) {
				$this->fields['account']['account_username'] = array(
					...
					'required'     => true,
					'placeholder'  => esc_attr__( 'Account #', 'woocommerce' ),
				);
			}
